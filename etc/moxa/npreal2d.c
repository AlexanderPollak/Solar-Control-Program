...
void ConnectSSL_G2( TTYINFO *infop );
int cert_set_path(SSL_CTX *ctx, char *ca_path);
EVP_PKEY * generate_key(void);
X509 *generate_x509(EVP_PKEY *pkey);
int write_to_disk(EVP_PKEY * pkey, X509 * x509, char *key_file_name, char *cert_file_name);
int gen_cert(void);
			...
			for (i = 0;i < ttys;i++)
			{
				...
				if (infop->sock_fd >= 0)
				{
					...
					if (infop->ssl_enable)
					{
						...
						if((infop->ssl_enable == SECURE_DATA_CMD) || (infop->ssl_enable == SECURE_DATA_CMD_CERT)) {
							SSL_shutdown(infop->pssl_g2);
							SSL_free(infop->pssl_g2);
							infop->pssl_g2 = NULL;
						}
					}
#endif
...
char *	cmdpath;
{
	...
	while ( ttys < MAX_TTYS )
	{
		...
		if((infop->ssl_enable == SECURE_DATA_CERT) || (infop->ssl_enable == SECURE_DATA_CMD_CERT))
			infop->cert_enable = 1;
		else
			infop->cert_enable = 0;
		infop->pssl_g2 = NULL;
...
system("logger 'TTY UNUSED'");
			...
			{ //cmd resp
				...
				if((infop->ssl_enable == SECURE_DATA_CMD) || (infop->ssl_enable == SECURE_DATA_CMD_CERT))
				{
					len = SSL_read(infop->pssl_g2,
							infop->sock_cmdbuf,
							CMD_BUFFER_SIZE);
				}
				else{
					len = read(infop->sock_cmd_fd,
						infop->sock_cmdbuf,
						CMD_BUFFER_SIZE);
				}
				...
				while (len > 0)
				{
										   ...
										   == ASPP_CMD_POLLING)
					{
						...
						if((infop->ssl_enable == SECURE_DATA_CMD) || (infop->ssl_enable == SECURE_DATA_CMD_CERT))
						{
							SSL_write(infop->pssl_g2, cmd_buf, len1);
						}
						else
						{
	...
	{	/* child process */
		...
		if ( connect(infop->sock_cmd_fd, (struct sockaddr*)&sock, sizeof(sock)) >= 0 )
		{
			...
			if ( connect(infop->sock_fd, (struct sockaddr*)&sock, sizeof(sock)) >= 0)
			{
				...
				{
#ifdef SSL_ON
					if((infop->ssl_enable != SECURE_DATA_CMD) && (infop->ssl_enable != SECURE_DATA_CMD_CERT))  // If it's G2 device, we don't need do anything here
#endif
					{
						int rand[16];
...
int verify_callback(int preverify_ok, X509_STORE_CTX *ctx)
{
    ...
    if (!preverify_ok) {
		sprintf(buf, "%s(%d) Verification error: %s\n",  __FUNCTION__, __LINE__, X509_verify_cert_error_string(err));
		log_event(buf);
    }
    return preverify_ok;
}
...
TTYINFO *	infop;
{
	...
	sprintf(workpath, "/usr/lib/npreal2/driver/certificate/");
	...
	if (infop->pssl==NULL)
	{
		if(infop->cert_enable)
		{
			cert_set_path(sslc_ctx, workpath);
		}
		...
		if (infop->pssl != NULL)
		{
			if(infop->cert_enable)
			{
				//log_event("pssl cert enable");
				SSL_set_verify(infop->pssl, SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT, NULL);
			}
			else
			{
				//log_event("pssl cert disable");
				SSL_set_verify(infop->pssl, SSL_VERIFY_NONE, NULL);
			}
	...
	if ((flags = fcntl(infop->sock_fd, F_GETFL, 0)) < 0) {
		sprintf(buf, "%s(%d) fcntl F_GETFL fail!",  __FUNCTION__, __LINE__);
		log_event(buf);
	}
	if (fcntl(infop->sock_fd, F_SETFL, flags | O_NONBLOCK) < 0) {
		sprintf(buf, "%s(%d) fcntl F_SETFL fail!",  __FUNCTION__, __LINE__);
		log_event(buf);
	}
	...
	if ( select(fd+1, &rfd, &wfd, 0, &tm) > 0 )
	{
		if ( FD_ISSET(fd, &wfd) || FD_ISSET(fd, &rfd))
		{
			...
			ret = SSL_connect(infop->pssl);
			pthread_mutex_unlock(&mutex);
			if (ret > 0)
			{
				if(infop->cert_enable)
				{
					X509* cert = SSL_get_peer_certificate(infop->pssl);
					if(cert)
					{
						X509_free(cert);
						/* verify certificate */
						long res = SSL_get_verify_result(infop->pssl);
						if(!(X509_V_OK == res))
						{
							sprintf(buf, "SSL_get_verify_result failed res = %d", res);
							log_event(buf);
							//disconntect
							//SSL_free(infop->pssl);
							//return -1;
						...
						{
							//log_event("SSL_get_verify_result pass");
							infop->ssl_time = 0;
							infop->state = STATE_RW_DATA;
						}
					}
					...
					{
						log_event("deamon can't get certificate");
						//disconnect
						//SSL_free(infop->pssl);
						//return -1;
						infop->state = STATE_TCP_CLOSE;
			...
			else
			{
				...
				sprintf(buf, "%s(%d) SSL_connect failed, SSL_get_error errno = %d", __FUNCTION__, __LINE__, ret);
				log_event(buf);
				switch (ret)
				{
...
TTYINFO *	infop;
{
	...
	if( infop->ssl_time_g2==0 ){
		for ( i=0, chk_infop=ttys_info; i<ttys; i+=1, chk_infop+=1 )
		{
			...
			if( (chk_infop->state==STATE_SSL_CONN) && (chk_infop->ssl_time_g2!=0)  ){
				//sprintf(mm, "logger \"CFD>(%d) Wait (%d) for SSL..\"", infop->tcp_port, chk_infop->tcp_port);
				//system(mm);
				return;
			}       
	...
	sysinfo(&sys_info);
	if( infop->ssl_time_g2==0 ){
		infop->ssl_time_g2 = sys_info.uptime;
	} else {
		//if ((sys_info.uptime - infop->ssl_time) < 3 ){
		//infop->ssl_time = sys_info.uptime;
		//      return;
	...
	//infop->ssl_time = sys_info.uptime;
	if (infop->pssl_g2==NULL)
	{
		if(infop->cert_enable)
		{
			cert_set_path(sslc_ctx, workpath);
		}
		infop->pssl_g2 = SSL_new(sslc_ctx);
		//sprintf(mm, "logger \"CFD>(%d) pssl=0x%X..\"", infop->tcp_port, infop->pssl);
		//system(mm);
		...
		if (infop->pssl_g2 != NULL)
		{
			if(infop->cert_enable)
			{
				//log_event("pssl_g2 cert enable");
				SSL_set_verify(infop->pssl_g2, SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT, NULL);
			}
			else
			{
				//log_event("pssl_g2 cert disable");
				SSL_set_verify(infop->pssl_g2, SSL_VERIFY_NONE, NULL);
			}

			if (SSL_set_fd(infop->pssl_g2, infop->sock_cmd_fd))
			{
				//sprintf(mm, "logger \"CFD>(%d) set_connect_state..\"", infop->tcp_port);
				//system(mm);
				SSL_set_connect_state(infop->pssl_g2);
			}
			else
			{
				//sprintf(mm, "logger \"CFD>(%d) SSL_set_fd() error..\"", infop->tcp_port);
	...
	if ((flags = fcntl(infop->sock_cmd_fd, F_GETFL, 0)) < 0) {
		sprintf(buf, "%s(%d) fcntl F_GETFL fail!",  __FUNCTION__, __LINE__);
		log_event(buf);
	}
	if (fcntl(infop->sock_cmd_fd, F_SETFL, flags | O_NONBLOCK) < 0) {
		sprintf(buf, "%s(%d) fcntl F_SETFL fail!",  __FUNCTION__, __LINE__);
		log_event(buf);
	}
	...
	if ( select(fd+1, &rfd, &wfd, 0, &tm) > 0 )
	{
		...
		{
			pthread_mutex_lock(&mutex);
			ret = SSL_connect(infop->pssl_g2);
			pthread_mutex_unlock(&mutex);
			if (ret > 0)
			{
				if(infop->cert_enable)
				{
					X509* cert = SSL_get_peer_certificate(infop->pssl_g2);
					if(cert)
					{
						X509_free(cert);
						/* verify certificate */
						int res = SSL_get_verify_result(infop->pssl_g2);
						if(!(X509_V_OK == res) && !(66 == res))
						{
							sprintf(buf, "%s(%d) certificate verify failed, errno = %d", __FUNCTION__, __LINE__, res);
							log_event(buf);
							infop->state = STATE_TCP_CLOSE;
							infop->reconn_flag = 0;
						}
					...
					{
						sprintf(buf, "%s(%d) certificate is NULL", __FUNCTION__, __LINE__);
						log_event(buf);
						infop->state = STATE_TCP_CLOSE;
						infop->reconn_flag = 0;				
					}
			...
			else
			{
				ERR_error_string_n(ERR_get_error(), buf, sizeof(buf));
				log_event(buf);
				ret = SSL_get_error(infop->pssl_g2, ret);
				sprintf(buf, "%s(%d) SSL_connect failed, SSL_get_error errno = %d", __FUNCTION__, __LINE__, ret);
				log_event(buf);
				switch (ret)
				{
...
int cert_set_path(SSL_CTX *ctx, char *ca_path)
{
	...
	X509_STORE *new_cert_store = X509_STORE_new();
	ret = X509_STORE_load_locations(new_cert_store, NULL, ca_path);
	if(ret != 1)
	{
		log_event("X509_STORE_load_locations failed");
		pthread_mutex_unlock(&mutex);
		return -1;
	}
	SSL_CTX_set_cert_store(ctx, new_cert_store);
	pthread_mutex_unlock(&mutex);
	return 0;
}
#endif // #ifdef SSL_ON
...
//#define CIPHER_LIST "DES-CBC3-SHA:AES128-SHA:AES256-SHA:ECDHE"
static void ssl_init(void)
{
	...
	sprintf(workpath, "%s", MX_LOCAL_CERT_FOLD);
	sprintf(certpath, "%s%s", MX_LOCAL_CERT_FOLD, MX_LOCAL_CERT_FILE);
	sprintf(keypath, "%s%s", MX_LOCAL_CERT_FOLD, MX_LOCAL_PRI_FILE);
	...
	if( gen_cert() < 0)
	{
		log_event("Generate local cert failed");
	}
	
#ifdef SSL_VER2
	//sslc_ctx = SSL_CTX_new(SSLv2_client_method());
	sslc_ctx = SSL_CTX_new(SSLv2_method());
#else
#ifdef SSL_VER3
	//sslc_ctx = SSL_CTX_new(SSLv3_client_method());
	sslc_ctx = SSL_CTX_new(SSLv3_method());
#else
	//sslc_ctx = SSL_CTX_new(SSLv23_client_method());
	sslc_ctx = SSL_CTX_new(SSLv23_method());
#endif
#endif

	// setup_client_ctx
	if(!SSL_CTX_use_certificate_file(sslc_ctx, certpath, SSL_FILETYPE_PEM))
	{
		log_event("SSL_CTX_use_certificate_file() error");
	}

	if(!SSL_CTX_use_PrivateKey_file(sslc_ctx, keypath, SSL_FILETYPE_PEM))
	{
		log_event("SSL_CTX_use_PrivateKey_file() error");
	}

	if(!SSL_CTX_check_private_key(sslc_ctx))
	{
		log_event("SSL_CTX_check_private_key() error");
	}

	SSL_CTX_set_verify(sslc_ctx, SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT, verify_callback);
	//SSL_CTX_set_verify(sslc_ctx, SSL_VERIFY_NONE, NULL);
	...
	// SSL_CTX_set_security_level was added in OpenSSL 1.1.0
	if( OPENSSL_VERSION_NUMBER >= 0x10100000L) {
		// set to Level 0 for compatibility with previous version of OpenSSL
		SSL_CTX_set_security_level(sslc_ctx, 0);
	}

	if(pthread_mutex_init(&mutex, NULL) != 0) {
		log_event("pthread_mutex_init failed");
	}
}
#endif
#ifndef STREAM
...
X509 *generate_x509(EVP_PKEY *pkey)
{
    ...
    if(!x509)
    {
        log_event("Unable to create X509 structure.");
        return NULL;
    }
	...
	/* certificate expiration date: 10 years from now (60s * 60m * 24h * 365d * 20y) */
    X509_gmtime_adj(X509_get_notBefore(x509), 0);
    X509_gmtime_adj(X509_get_notAfter(x509), 630720000);
	...
	char str[64];
	srand(time(NULL));
	sprintf(str, "NPortRealtty%ld", rand());
    /* Set the country code and common name. */
    X509_NAME_add_entry_by_txt(name, "C",  MBSTRING_ASC, (unsigned char *)"CA",        -1, -1, 0);
    X509_NAME_add_entry_by_txt(name, "O",  MBSTRING_ASC, (unsigned char *)"MOXA", -1, -1, 0);
    X509_NAME_add_entry_by_txt(name, "CN", MBSTRING_ASC, (unsigned char *)str, -1, -1, 0);
    ...
    {
        log_event("Signing certificate failed.");
        X509_free(x509);
        return NULL;
    }
...
int write_to_disk(EVP_PKEY * pkey, X509 * x509, char *key_file_name, char *cert_file_name)
{
    ...
    FILE * pkey_file = fopen(key_file_name, "wb");
    if(!pkey_file)
    {
        log_event("Unable to open \"key.pem\" for writing.");
        return -1;
    }
    ...
    if(!ret)
    {
        log_event("Unable to write private key to disk.");
        return -2;
    }
    
    /* Open the PEM file for writing the certificate to disk. */
    FILE * x509_file = fopen(cert_file_name, "wb");
    if(!x509_file)
    {
        log_event("Unable to open \"cert.pem\" for writing.");
        return -3;
    }
    ...
    if(!ret)
    {
        log_event("Unable to write certificate to disk.");
        return -4;
    }
...
int gen_cert(void)
{
	char	workpath[128], certpath[128], keypath[128];
	...
	do
	{
		if( (certfile==NULL) || (keyfile==NULL) )
		{
			...
			if(!pkey)
			{
				log_event("Generating RSA key failed.");
				iret = -1;
				break;
			}
			...
			if(!x509)
			{
				log_event("Generating certificate failed.");
				EVP_PKEY_free(pkey);
				iret = -2;
				break;
			}
			...
			ret = write_to_disk(pkey, x509, keypath, certpath);
		    EVP_PKEY_free(pkey);
		    X509_free(x509);
		    if(ret == 0)
		    {
	    	...
	    	{
	    		log_event("Writing key and certificate to file failed.");
		        iret = -3;
				break;
	    	}
			...
			if(p==NULL)
			{
				log_event("open private key file error");
				iret = -4;
				break;
			}
			evp_key = PEM_read_PrivateKey(p, NULL, NULL, NULL);
			if(evp_key==NULL)
			{
				log_event("PEM_read_PrivateKey() error");
				fclose(p);
				iret = -5;
				break;
			}
			...
			ret = PEM_write_bio_PrivateKey(bio, evp_key, NULL, NULL, 0, 0, NULL);
			if(!ret)
			{
				log_event("PEM_write_bio_PrivateKey() error");
				fclose(p);
				BIO_free_all(bio);
				iret = -6;
				break;
			}
			ret = PEM_write_bio_PUBKEY(bio, evp_key);
			if(!ret)
			{
				log_event("PEM_write_bio_PUBKEY() error");
				fclose(p);
				BIO_free_all(bio);
				iret = -6;
				break;
