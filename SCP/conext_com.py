...
class XW():
    ...
    def read_Inverter_All(self):
        ...
        try:
            ...
            XW_list[1][0] = self.read_Grid_Voltage()
            XW_list[2][0] = self.read_Grid_Current()
            XW_list[3][0] = self.read_Grid_Power()
            XW_list[4][0] = self.read_Grid_Frequency()
            XW_list[5][0] = self.read_Load_Voltage()
            XW_list[6][0] = self.read_Load_Current()
            XW_list[7][0] = self.read_Load_Power()
            XW_list[8][0] = self.read_Load_Frequency()
            XW_list[9][0] = self.read_Invert_DC_Current()
            XW_list[10][0] = self.read_Invert_DC_Power()
            XW_list[11][0] = self.read_Energy_Grid_Month()
            XW_list[12][0] = self.read_Energy_Load_Month()
            XW_list[13][0] = self.read_Energy_Battery_Month()
            XW_list[14][0] = self.read_Low_Battery_Cut_Out()
            XW_list[15][0] = self.read_Low_Battery_Cut_Out_Delay()
            XW_list[16][0] = self.read_Hysteresis()
            XW_list[17][0] = self.read_Inverter_Status()
            XW_list[18][0] = self.read_Inverter_Active_Warning()
            XW_list[19][0] = self.read_Inverter_Active_Fault()
            XW_list[20][0] = self.read_Grid_Support_Status()
            XW_list[21][0] = self.read_Load_Shave_Status()
